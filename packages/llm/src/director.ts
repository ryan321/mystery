...
10. You may include suggestedPatch with setLocationId / addEvidenceIds / setFlags / accuse — but only for ids in the pack. Prefer intents; patch is optional. Only set suggestedPatch.setLocationId for a genuine player-directed move (2b) — never to relocate the player toward a person or object. Do NOT set progress/story flags (found_*, opened_*, unlocked_*, *_seen, *_found and the like) or grant evidence for a discovery: those are the engine's to apply from the authored result of an inspect/use, and it drops flag writes it did not author. Never suggestedPatch when using a blocked_* note.
